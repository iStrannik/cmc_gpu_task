...
struct Operator{
    ...
    Operator(double* operatorA_a, double* operatorA_b, double* operatorA_f, Grid* grid) {
        ...
        this->send_buffer = new double[2 * (grid->global_step_x + grid->global_step_y)];
...
void extend_matrix(Operator* A, double* w, double* extended_w, int tag) {
    ...
    if (A->grid->neighbours_ranks[0] != -1) { // left
        ...
        MPI_Isend(A->send_buffer, A->grid->global_step_y, MPI_DOUBLE, A->grid->neighbours_ranks[0], tag, MPI_COMM_WORLD, &(requests[cnt_requests++]));
        MPI_Irecv(extended_w, A->grid->global_step_y, MPI_DOUBLE, A->grid->neighbours_ranks[0], tag, MPI_COMM_WORLD, &(requests[cnt_requests++]));
    }

    if (A->grid->neighbours_ranks[1] != -1) { // top
        for (size_t j = 0; j < A->grid->global_step_x; ++j) {
            A->send_buffer[A->grid->global_step_y + j] = w[j * A->grid->global_step_y + A->grid->global_step_y - 1];
        }
        MPI_Isend(&(A->send_buffer[A->grid->global_step_y]), A->grid->global_step_x, MPI_DOUBLE, A->grid->neighbours_ranks[1], tag, MPI_COMM_WORLD, &(requests[cnt_requests++]));
        MPI_Irecv(&(extended_w[A->grid->global_step_y]), A->grid->global_step_x, MPI_DOUBLE, A->grid->neighbours_ranks[1], tag, MPI_COMM_WORLD, &(requests[cnt_requests++]));
    }

    if (A->grid->neighbours_ranks[2] != -1) { // right
        for (size_t j = 0; j < A->grid->global_step_y; ++j) {
            A->send_buffer[A->grid->global_step_y + A->grid->global_step_x + j] = w[(A->grid->global_step_x - 1) * A->grid->global_step_y + j];
        }   
        MPI_Isend(&(A->send_buffer[A->grid->global_step_y + A->grid->global_step_x]), A->grid->global_step_y, MPI_DOUBLE, A->grid->neighbours_ranks[2], tag, MPI_COMM_WORLD, &(requests[cnt_requests++]));
        MPI_Irecv(&(extended_w[A->grid->global_step_y + A->grid->global_step_x]), A->grid->global_step_y, MPI_DOUBLE, A->grid->neighbours_ranks[2], tag, MPI_COMM_WORLD, &(requests[cnt_requests++]));
    }

    if (A->grid->neighbours_ranks[3] != -1) { // down
        for (size_t j = 0; j < A->grid->global_step_x; ++j) {
            A->send_buffer[2 * A->grid->global_step_y + A->grid->global_step_x + j] = w[A->grid->global_step_y * j];
        }

        MPI_Isend(&(A->send_buffer[2 * A->grid->global_step_y + A->grid->global_step_x]), A->grid->global_step_x, MPI_DOUBLE, A->grid->neighbours_ranks[3], tag, MPI_COMM_WORLD, &(requests[cnt_requests++]));
        MPI_Irecv(&(extended_w[2 * A->grid->global_step_y + A->grid->global_step_x]), A->grid->global_step_x, MPI_DOUBLE, A->grid->neighbours_ranks[3], tag, MPI_COMM_WORLD, &(requests[cnt_requests++]));
    }
...
void apply_operator(Operator* A, double* w, double* r, bool subtractF) {
    ...
    for (size_t idx = 0; idx < A->grid->global_step_x * A->grid->global_step_y; ++idx) {
        int i = idx / A->grid->global_step_y;
        int j = idx % A->grid->global_step_y;
...
double scalar_product(double* a, double* b, Grid* grid) {
    ...
    for (size_t idx = 0; idx < grid->global_step_x * grid->global_step_y; ++idx) {
        result += a[idx] * b[idx];
    }
