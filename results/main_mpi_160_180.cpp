...
- Всё так, была ошибка в openmp реализации. В одном месте при создании матриц A и B выходил за границу массива, из-за чего элементы из кучи брались.
В текущей реализации openmp это исправлено.

- Да. Например надо разделить сторону 92 на 3 процесса. Тогда стороны прямоугольников у каждого из них будут 31 31 30 вдоль этой координаты. То есть в алгоритме между процессами равномерно распределяется остаток от деления стороны на количество процессов.

- Замечу, что матрицы A и B пересылаются разово в подготовительной стадии. Я отталкивался от идеи, что каждый из процессов владеет какой-то своей части матрицы при генерации. Когда понадобилось расширить матрицы по границе, то был выбор - либо заново писать код, либо воспользоваться механизмом передачи - так получилось и логичнее и лаконичней на мой взгляд. Ещё гипотетический бонус пересылки границ - в случае если бы генерация элементов матриц была бы зататратной операцией, возможно пересылка была бы быстрее.
