...
struct Grid{
    ...
    Grid(Point start, double step_x, double step_y, size_t M, size_t N) {
        ...
        this->M = M;
        this->N = N;
        this->grid_x = generate_grid_vector(start.x, step_x, M);
        this->grid_y = generate_grid_vector(start.y, step_y, N);
...
int main () {
    ...
    do {
        ...
        double rr = scalar_product(r, r, &grid);
        double Arr = scalar_product(Ar, r, &grid);
