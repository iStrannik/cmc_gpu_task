...
void extend_matrix(Operator* A, double* w, double* extended_w, int tag) {
    ...
    if (A->grid->neighbours_ranks[0] != -1) { // left
        double send_operator[A->grid->global_step_y];
        for (size_t j = 0; j < A->grid->global_step_y; ++j) {
            send_operator[j] = w[j];
        }
        MPI_Sendrecv(send_operator, A->grid->global_step_y, MPI_DOUBLE, A->grid->neighbours_ranks[0], tag, 
                     extended_w, A->grid->global_step_y, MPI_DOUBLE, A->grid->neighbours_ranks[0], tag, 
                    MPI_COMM_WORLD, &status);
    }

    if (A->grid->neighbours_ranks[1] != -1) { // top
        double send_operator[A->grid->global_step_x];
        for (size_t j = 0; j < A->grid->global_step_x; ++j) {
            send_operator[j] = w[j * A->grid->global_step_y + A->grid->global_step_y - 1];
        }

        MPI_Sendrecv(send_operator, A->grid->global_step_x, MPI_DOUBLE, A->grid->neighbours_ranks[1], tag, 
    ...
    if (A->grid->neighbours_ranks[2] != -1) { // right
        double send_operator[A->grid->global_step_y];
        for (size_t j = 0; j < A->grid->global_step_y; ++j) {
            send_operator[j] = w[(A->grid->global_step_x - 1) * A->grid->global_step_y + j];
        }
        MPI_Sendrecv(send_operator, A->grid->global_step_y, MPI_DOUBLE, A->grid->neighbours_ranks[2], tag, 
    ...
    if (A->grid->neighbours_ranks[3] != -1) { // down
        double send_operator[A->grid->global_step_x];
        for (size_t j = 0; j < A->grid->global_step_x; ++j) {
            send_operator[j] = w[A->grid->global_step_y * j];
        }

        MPI_Sendrecv(send_operator, A->grid->global_step_x, MPI_DOUBLE, A->grid->neighbours_ranks[3], tag, 
...
void apply_operator(Operator* A, double* w, double* r, bool subtractF) {
    double extended_w[2 * (A->grid->global_step_x + A->grid->global_step_y)];
    extend_matrix(A, w, extended_w, 0x63);
    // #pragma omp parallel for collapse(2)
    for (size_t i = 0; i < A->grid->global_step_x; ++i) {
        for (size_t j = 0; j < A->grid->global_step_y; ++j) {
            ...
            r[i * A->grid->global_step_y + j] = 
                (-1.0 / A->grid->step_x) * (
                    get_by_index(A, A->operatorA_a, A->extended_A, i + 1, j) * (get_by_index(A, w, extended_w, i + 1, j) - get_by_index(A, w, extended_w, i, j)) / A->grid->step_x -
                    get_by_index(A, A->operatorA_a, A->extended_A, i, j) * (get_by_index(A, w, extended_w, i, j) - get_by_index(A, w, extended_w, i - 1, j)) / A->grid->step_x
                ) +
                (-1.0 / A->grid->step_y) * (
                    get_by_index(A, A->operatorA_b, A->extended_B, i, j + 1) * (get_by_index(A, w, extended_w, i, j + 1) - get_by_index(A, w, extended_w, i, j)) / A->grid->step_y -
                    get_by_index(A, A->operatorA_b, A->extended_B, i, j) * (get_by_index(A, w, extended_w, i, j) - get_by_index(A, w, extended_w, i, j - 1)) / A->grid->step_y
                );
        }
    }
    if (subtractF) {
...
int main(int argc, char **argv) {
    ...
    // std::cout << myrank << " x: " << x_start << " y: " << y_start << " size x: " << current_dimension_size_x << " size y " << current_dimension_size_y << std::endl;

    // std::cout << myrank << " left " << neighbours_ranks[0] << " top " << neighbours_ranks[1] << " right " << neighbours_ranks[2] << " down " << neighbours_ranks[3] << std::endl;
    ...
    /*
    for (size_t j = 0; j < current_dimension_size_x; ++j) {
        std::cout << std::fixed << std::setprecision(2) << grid.grid_x[j] << " ";
    }
    std::cout << std::endl;
    */
    ...
    char a[100];
    sprintf(a, "x_%d_y_%d.txt", x_start, y_start);

    std::ofstream out;
    out.open(a);
    for (size_t j = 0; j < grid.global_step_y; ++j) {
        for (size_t i = 0; i < grid.global_step_x; ++i) {
            out << operatorA->operatorA_a[i * grid.global_step_y + j] << " ";
        }
        out << std::endl;
    }
    out.close();
